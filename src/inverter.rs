...
impl PVInverter {
    ...
    async fn get_battery_charge_voltage(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0014, 1).await?[0] as i16;
        let value = f32::from(value);
        Ok(value / 10.0)
    }

    async fn get_battery_charge_current(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0015, 1).await?[0] as i16;
        let value = f32::from(value);
        Ok(value / 10.0)
    }

    async fn get_battery_charge_power(self: &mut Self) -> Result<i16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x0016, 1).await?[0] as i16)
    }

    async fn get_battery_soc(self: &mut Self) -> Result<i16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x001c, 1).await?[0] as i16)
    }

    async fn get_battery_discharge_max_current(self: &mut Self) -> Result<i16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x0025, 1).await?[0] as i16)
    }

    async fn get_battery_power_all(self: &mut Self) -> Result<i32, std::io::Error> {
        let result = self.modbus_ctx.read_input_registers(0x01FA, 2).await?;
        Ok(((result[1] as i32) << 16) | result[0] as i32)
    }

    async fn get_battery_charge_discharge_power(self: &mut Self) -> Result<i32, std::io::Error> {
        let result = self.modbus_ctx.read_input_registers(0x0114, 2).await?;
        Ok(((result[1] as i32) << 16) | result[0] as i32)
    }

    async fn get_feedin_power(self: &mut Self) -> Result<i32, std::io::Error> {
        let result = self.modbus_ctx.read_input_registers(0x0046, 2).await?;
        Ok(((result[1] as i32) << 16) | result[0] as i32)
    }

    async fn get_battery_temperature(self: &mut Self) -> Result<i16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x0018, 1).await?[0] as i16)
    }

    async fn get_user_soc(self: &mut Self) -> Result<u16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x00BE, 1).await?[0])
    }

    async fn get_radiator_temperature(self: &mut Self) -> Result<i16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x0008, 1).await?[0] as i16)
    }

    async fn get_pv_power_1(self: &mut Self) -> Result<u16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x000A, 1).await?[0])
    }

    async fn get_pv_power_2(self: &mut Self) -> Result<u16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x000B, 1).await?[0])
    }

    async fn get_pv_voltage_1(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0003, 1).await?[0];
        let value = f32::from(value);
        Ok(value / 10.0)
    }

    async fn get_pv_voltage_2(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0004, 1).await?[0];
        let value = f32::from(value);
        Ok(value / 10.0)
    }

    async fn get_pv_current_1(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0005, 1).await?[0];
        let value = f32::from(value);
        Ok(value / 10.0)
    }

    async fn get_pv_current_2(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0006, 1).await?[0];
        let value = f32::from(value);
        Ok(value / 10.0)
    }

    async fn get_grid_current(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0001, 1).await?[0];
        let value = f32::from(value as i16);
        Ok(value / 10.0)
    }

    async fn get_grid_power(self: &mut Self) -> Result<i16, std::io::Error> {
        Ok(self.modbus_ctx.read_input_registers(0x0002, 1).await?[0] as i16)
    }

    async fn get_grid_frequency(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0007, 1).await?[0];
        let value = f32::from(value);
        Ok(value / 100.0)
    }

    async fn get_grid_voltage(self: &mut Self) -> Result<f32, std::io::Error> {
        let value = self.modbus_ctx.read_input_registers(0x0000, 1).await?[0];
        let value = f32::from(value);
        Ok(value / 10.0)
    }

    async fn get_machine_type(self: &mut Self) -> Result<u16, std::io::Error> {
        Ok(self.modbus_ctx.read_holding_registers(0x0105, 1).await?[0])
    }

    async fn get_serial_number(self: &mut Self) -> Result<String, std::io::Error> {
